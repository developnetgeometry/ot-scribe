...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const result = await sendEmployeeNotification(supabase, payload.requestId, payload.notificationType);
 ...
 */ async function sendEmployeeNotification(supabase, requestId, notificationType) {
  ...
  const { data: otRequest, error: otError } = await supabase.from('ot_requests').select('id, employee_id, ot_date, total_hours, status, supervisor_id, hr_id, supervisor_remarks, hr_remarks, management_remarks').eq('id', requestId).single();
  ...
  ;
  if (approverId) {
    const { data: approver, error: approverError } = await supabase.from('profiles').select('full_name').eq('id', approverId).single();
    if (!approverError && approver) {
      approverName = approver.full_name;
    }
  }
  // 5. Build notification content based on type
  const notificationPayload = buildNotificationPayload(otRequest, notificationType, approverName);
 ...
 */ function determineApproverId(otRequest) {
 ...
 */ function buildNotificationPayload(otRequest, notificationType, approverName) {
  ...
  if (notificationType === 'approved') {
    ...
    return {
      ...
      badge: '/icons/badge-approved.svg',
      notification_type: 'ot_requests_approved',
      data: {
        targetUrl,
    ...
    const bodyText = rejectionReason ? `${formattedDate} - ${otRequest.total_hours} hours request rejected: ${rejectionReason}` : `${formattedDate} - ${otRequest.total_hours} hours request rejected`;
    return {
      user_id: otRequest.employee_id,
      title: 'OT Request Rejected',
      body: bodyText,
      icon: '/icons/icon-192x192.png',
      badge: '/icons/badge-rejected.svg',
      notification_type: 'ot_requests_rejected',
      data: {
        targetUrl,
 ...
 */ function getRejectionReason(otRequest) {
