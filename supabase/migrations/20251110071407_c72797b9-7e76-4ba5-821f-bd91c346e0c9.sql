-- Fix variable name collision in calculate_daily_ot_distribution
DROP FUNCTION IF EXISTS public.calculate_daily_ot_distribution(uuid, date, day_type);
...
-- Recalculate all existing OT requests to apply the fix
DO $$
DECLARE
  rec RECORD;
BEGIN
  FOR rec IN 
    SELECT DISTINCT employee_id, ot_date 
    FROM ot_requests 
